...
def heatmap_with_time(df: pd.DataFrame, geo_wkt_col: str, time_col: str, agg_unit: str = MINUTE_AGG) -> HeatMapWithTime:
    ...
    time_index = sorted(list(time_groupby_dict.keys()))  # type: ignore
    data = []
    for t in time_index:
        data.append(
...
def get_image_overlay(points: List[Point], scores: np.array, step: float = 50,
                      name: str = None) -> folium.raster_layers.ImageOverlay:
    ...
    # if return_array:  # noqa
    #     return z, (lons_index, lats_index)  # noqa
    z = cm(z)
